/* ============================================================
   SK'S DENTAL LOUNGE & MAXILLOFACIAL HOSPITAL — main.js
...
const HEADLINE_LINE1 = "SK's Dental Lounge";
const HEADLINE_LINE2 = '& Maxillofacial Hospital';
...
function typeHeadline() {
  ...
  line1El.className = 'headline-line1 shiny-text';
...
const bookings = [
  { name: 'Duty Oral Surgeon', desc: 'Just completed an emergency maxillofacial consultation.', initial: 'D' },
  { name: 'Senior Consultant', desc: 'Scheduled a precision veneer restoration treatment.', initial: 'S' },
  { name: 'SK\'s Dental Lounge Reception', desc: 'Booked a new patient consultation via WhatsApp.', initial: 'S' },
  { name: 'Duty Maxillofacial Specialist', desc: 'Successfully completed a complex surgical extraction.', initial: 'D' },
  { name: 'Senior Surgeon', desc: 'Completed a precision pain-free Root Canal Treatment (RCT).', initial: 'S' },
  { name: 'Dental Support Desk', desc: 'Answered a query about our 24/7 emergency availability.', initial: 'D' }
];
...
(function initHeroCardSwap() {
  ...
  const delay = 3500; // Delay in milliseconds between swaps (3.5 seconds per user request)
  ...
  function swap() {
    ...
    // Synchronously update order array for the NEXT swap call to prevent overlap glitches
    const front = order[0];
    ...
    // Force previous timeline to completion instantly if it's still running
    if (tl && tl.isActive()) {
      tl.progress(1);
    }
  ...
  // Pause on hover (only for mouse pointer inputs to prevent mobile touch issues)
  ...
  cards.forEach(card => {
    card.addEventListener('mousedown', (e) => {
      // Don't trigger card scale effect if clicking a link inside
  ...
  // Prevent card scaling & drag conflicts when clicking links inside cards
  cards.forEach(card => {
